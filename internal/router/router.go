...
func New_Router(db *sql.DB) http.Handler {
    ...
    chat_h := &handler.Chat_handler{DB: db} // NEW
    ...
    m.HandleFunc("/api/users", post_h.Auth_middleware(chat_h.Get_users)) // NEW
    m.HandleFunc("/api/messages/", post_h.Auth_middleware(chat_h.Get_messages)) // NEW
